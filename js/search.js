 ...
 *
 * Versión adaptada para funcionar con la navbar dinámica
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('navbarReady', initSearchModule);

    // También intentar inicializar después de un tiempo como respaldo
    setTimeout(function() {
        const searchIcon = document.querySelector('.search-icon');
        if (searchIcon && !window.searchAPI) {
            console.log('Inicializando search.js tras timeout');
            initSearchModule();
        }
    }, 500);

    // Función principal de inicialización del módulo de búsqueda
    function initSearchModule() {
        ...
        function performSearch(query) {
            ...
            if (typeof window.productos !== 'undefined') {
                productosDisponibles = window.productos;
            } else {
        ...
        console.log('Módulo de búsqueda inicializado correctamente');
    }
});
